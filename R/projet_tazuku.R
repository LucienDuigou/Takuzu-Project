...
#' 
#' Catte fonction cree 3 listes contenant chacune 20 grille de jeu Takuzu de difficulte facile, moyen et difficile
#' @return 3 vecteurs contenant des grilles de taille 8x8
#' examples
#' data_base()
#' @export
data_base = function(){
...
while(length(DB_facile)<20){
  ...
  if (any(DB_facile==P)==FALSE){
    DB_facile = append(DB_facile, list(P))
  }
}

while(length(DB_moyen)<20){
  G=create_game()
  P=as.vector(t(G))
  v=c()
  while (length(v)<29){
  ...
  P_ <- (matrix(vect, ncol = 8, byrow = TRUE))
  if (any(DB_moyen==P)==FALSE){
    DB_moyen = append(DB_moyen, list(P))
  }
}

while(length(DB_difficile)<20){
  G=create_game()
  P=as.vector(t(G))
  v=c()
  while (length(v)<32){
  ...
  if (any(DB_difficile==P)==FALSE){
    DB_difficile = append(DB_difficile, list(P))
  }
}
return(c(DB_facile,DB_moyen,DB_difficile))
}

utils::globalVariables(data_base())
